        ...
        private void LoadData_Item()
        {
            try
            {
                ...
                else
                {
                    ...
                    if (PageData != null)
                    {
                        UserId = PageData.IsPageOnwer != null && PageData.IsPageOnwer.Value ? PageData.LastMessage.LastMessageClass.FromId == UserDetails.UserId ? PageData.LastMessage.LastMessageClass.ToId : PageData.LastMessage.LastMessageClass.FromId : UserDetails.UserId ?? UserDetails.UserId;
                        ...
                        ActionBarTitle.Text = Methods.FunString.DecodeString(PageData.PageName);
                        SayHiToTextView.Text = Methods.FunString.DecodeString(PageData.PageName);
                    }
                }
            ...
            try
            {
                if (TaskWork == "Working")
                {
                    ...
                    if (Methods.CheckConnectivity())
                    {
                        ...
                        (int apiStatus, var respond) = await RequestsAsync.PageChat.FetchPageChatMessages(PageId, UserId, "0", lastMessageId);
                        if (apiStatus == 200)
                        {
                            int countList = MAdapter.DifferList.Count;
                            if (respond is PageMessagesObject result)
